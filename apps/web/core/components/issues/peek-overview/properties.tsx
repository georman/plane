...
// i18n
import { brandName, useTranslation } from "@plane/i18n";
// ui icons
import {
  CycleIcon,
  StatePropertyIcon,
...
import { IssueCustomFields } from "@/components/issues/issue-detail/custom-fields";
  ...
  return (
    <div>
      ...
      <div className={`mt-3 w-full space-y-3 ${disabled ? "opacity-60" : ""}`}>
        ...
        {createdByDetails && (
          ...
          >
            ...
            <span className="grow truncate text-body-xs-medium leading-5 text-secondary">
              {createdByDetails?.display_name?.includes("-intake") ? `${brandName()}` : createdByDetails?.display_name}
            </span>
          </SidebarPropertyListItem>
        )}
        ...
        {/* GAM: the project's custom fields */}
        <IssueCustomFields workspaceSlug={workspaceSlug} projectId={projectId} issueId={issueId} disabled={disabled} />
      </div>
    </div>
  );
});
